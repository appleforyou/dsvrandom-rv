...
require_relative '../rv/ooe_checker'
  ...
  def rv_place_non_progression_pickups(checker)
    ...
    remaining_locations.each_with_index do |loc, i|
      if loc[:container] == ("Spell")
        # Boss
        pickup_id = get_unplaced_non_progression_skill()
      elsif GAME == "ooe" && loc[:id] == "08-02-06_01"
      ...
      #spoiler_log.puts pickup_id.inspect
      #spoiler_log.puts "Placing #{(OoEItems.items.find {|key, value| value[:id] == pickup_id})[:name]} at #{loc[:room]}"
      rv_change_entity_location_to_pickup_id(loc, pickup_id)
    end
  end
  ...
  def rv_change_entity_location_to_pickup_id(location, pickup_id)
    ...
    if GAME == "ooe" && location[:id] == "08-02-06_01" # Strength Ring blue chest spawned by the searchlights after you kill the Tin Man
      if entity.var_a != 2
  ...
  def rv_place_progression_pickups(checker, &block)
    ...
    spoiler_log.puts game.areas[0x1E]
    custos_dominus = ["Dominus Hatred", "Dominus Anger", "Dominus Agony", "Sinestro Custos", "Dextro Custos", "Arma Custos"]
    hints_to_use = custos_dominus.shuffle.first(3)
    cats = [0x756, 0x758, 0x75A]
    if GAME == "ooe"
      # Give cats a hint for a custos or dominus glyph
      #Placing Tofu (Monastery) in Argila Swamp
      room = game.areas[5].sectors[0].rooms[0]
      cat = Entity.new(room, room.fs)
      ...
      #Remove the cat in its original position since it's not needed there.
      original_cat = game.areas[0x12].sectors[0].rooms[7].entities[7]
      original_cat.type = 0
      original_cat.write_to_rom()

      #Placing Ink (Manor) in Oblivion Ridge
      room = game.areas[0x10].sectors[0].rooms[0]
      cat = Entity.new(room, room.fs)
      ...
      #Remove the cat in its original position since it's not needed there.
      original_cat = game.areas[0x0E].sectors[0].rooms[3].entities[0]
      original_cat.type = 0
      original_cat.write_to_rom()

      # Placing SoyBeanFlour (Minera) in George's position
      room = game.areas[0x11].sectors[0].rooms[8]
      cat = Entity.new(room, room.fs)
      ...
      #Remove the cat in its original position since it's not needed there.
      original_cat = game.areas[8].sectors[2].rooms[1].entities[0x10]
      original_cat.type = 0
      original_cat.write_to_rom()
    end
    ...
    while true
      ...
      checker.add_item(pickup_name)
      puts hints_to_use
      if GAME == "ooe" && hints_to_use.include?(pickup_name)
        pickup_vague_area = ""
        ...
        newlines = game.text_database.text_list[cat_to_use].decoded_string.split(/((?:\{[^}]+\})+)/)
        ...
        spoiler_log.puts "Placing #{pickup_str} on #{cat_to_use}"
      end
  ...
  def randomize_pickups_completably(checker, &block)
    ...
    total_progression_pickups = checker.all_progression_pickups.length
    place_progression_pickups(checker) do |progression_pickups_placed|
      percent_done = progression_pickups_placed.to_f / total_progression_pickups
      yield percent_done
    end
